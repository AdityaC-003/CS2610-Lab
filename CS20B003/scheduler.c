/* 
    CS2610 Lab #6 - DRAM Row Buffer Management
    Aditya C (CS20B003) & Kruthic Vignesh M (CS20B045) 
*/
...
void inc_counter()              //method to increment 4-bit saturating counter
{
    if(counter < COUNT_MAX)
        counter++;
}

void dec_counter()              //method to decrement 4-bit saturating counter
{
...
void schedule(int channel)
{
	...
	if(counter > HIGH_COUNT)            //switch to closed policy if counter exceeds HIGH_THRESHOLD
    {
        policy_switches++;
        paging_policy = CLOSED;
    }

    if(counter < LOW_COUNT)             //switch to closed policy if counter falls below LOW_THRESHOLD
    {
        policy_switches++;
        paging_policy = OPEN;
    }
			...
			if(wr_ptr->command_issuable)
			{
                if(paging_policy == CLOSED)
                {
                    ...
                    if (wr_ptr->next_command == COL_WRITE_CMD)
                    {
                        ...
                        if(wr_ptr->dram_addr.row == prev_active_row[channel][wr_ptr->dram_addr.rank][wr_ptr->dram_addr.bank])   //if page hit, update counter
                        {                           
                            dec_counter();
                            closed_hits++;
                        }
                        else
                            prev_active_row[channel][wr_ptr->dram_addr.rank][wr_ptr->dram_addr.bank] = wr_ptr->dram_addr.row;   //update prev_active_row
                ...
                {
                    open_accesses++;
                    if(wr_ptr->next_command == COL_WRITE_CMD && wr_ptr->dram_addr.row != prev_active_row[channel][wr_ptr->dram_addr.rank][wr_ptr->dram_addr.bank])      //if page miss, update counter
                    {
                        inc_counter();
                        open_misses++;
                        prev_active_row[channel][wr_ptr->dram_addr.rank][wr_ptr->dram_addr.bank] = wr_ptr->dram_addr.row;       //update prev_active_row
		...
		LL_FOREACH(read_queue_head[channel],rd_ptr)
		{
			if(rd_ptr->command_issuable)
			{
                if(paging_policy == CLOSED)
                {
				   ...
				   If the bank just did an activate or precharge, it is not a candidate for closure. */
                    if (rd_ptr->next_command == COL_READ_CMD)
                    {
                        ...
                        if(rd_ptr->dram_addr.row == prev_active_row[channel][rd_ptr->dram_addr.rank][rd_ptr->dram_addr.bank])       //if page hit, update counter
                        {
                            dec_counter();
                            closed_hits++;
                        }
                        else
                            prev_active_row[channel][rd_ptr->dram_addr.rank][rd_ptr->dram_addr.bank] = rd_ptr->dram_addr.row;       //update prev_active_row
                ...
                else
                {
                    ...
                    if(rd_ptr->next_command == COL_READ_CMD && rd_ptr->dram_addr.row != prev_active_row[channel][rd_ptr->dram_addr.rank][rd_ptr->dram_addr.bank])       //if page miss, update counter
                    {
                        inc_counter();
                        open_misses++;
                        prev_active_row[channel][rd_ptr->dram_addr.rank][rd_ptr->dram_addr.bank] = rd_ptr->dram_addr.row;       //update prev_active_row
    ...
    if(paging_policy == CLOSED)             //do Precharging if following closed page policy
    {
